...
const DRAGGABLE_OPTIONS: OrBoolean<DraggableOptions> = {
    modifiers: [
...
export class Display {
    ...
    constructor(udid: string, state: DeviceState) {
        this.scale = BASE_SIZE;
        const { widthPixels = 32, heightPixels = 32, density = 1 } =
            state.display || {};
        const width = (widthPixels / density) * 2;
        const height = (heightPixels / density) * 2;
     ...
     */
    static onDragMove(event: InteractEvent) {
     ...
     * @param {interact.InteractEvent} event
     */
    static onDragStart(event: InteractEvent) {
        const display = Display.get(event.target)!;
