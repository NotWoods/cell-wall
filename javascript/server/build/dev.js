...
function resize(image, { width, height }, query = {}) {
  ...
  console.log(width, height, alignBits, resizeMode);
...
function attachWebsocketHandlers(server, websocketHandlers) {
  const webSocketServers = new WeakMap(websocketHandlers.map((handler) => {
  ...
  const otherHandlers = websocketHandlers.filter((handler) => {
  ...
  server.on("upgrade", (request, socket, head) => {
    ...
    if (!handler) {
      handler = otherHandlers.find((handler2) => handler2.path(pathname));
    }
    if (!handler) {
      socket.destroy();
      return;
