import { c as create_ssr_component, s as subscribe, e as escape, a as add_attribute, f as each, v as validate_component, j as add_classes, n as noop } from './ssr-966736c7.js';
import { s as splitToBuckets, f as fitScale, a as applyScale } from './fit-scale-333a67b3.js';
import { d as derived, r as readable } from './index2-505c6d08.js';
import { c as cellCanvas, b as blankState } from './web-ec6e3b01.js';
...
const SelectAppContent = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `<h3 class="text-md">${escape(info.deviceName || info.serial)}</h3> <a href="${"/remote/custom?id=" + escape(info.serial, true)}" class="${[
...
const SelectAppCard = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `<article class="bg-slate-800 p-2 border rounded border-slate-700 flex flex-col absolute box-border overflow-hidden" style="${"left: " + escape(rect.x, true) + "px; top: " + escape(rect.y, true) + "px; width: " + escape(rect.width, true) + "px; height: " + escape(rect.height, true) + "px;"}">${validate_component(SelectAppContent, "SelectAppContent").$$render($$result, { info, state, small }, {}, {})}</article>`;
});
const css = {
...
const Page = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `<section class="relative" style="${"height: " + escape(heightStyle, true)}"${add_attribute("this", container, 0)}>${each(buckets.rectWithPos, (info) => {
    return `${validate_component(SelectAppCard, "SelectAppCard").$$render(
      $$result,
      {
        info,
        scale,
        state: $remoteState.get(info.serial)?.state
      },
      {},
      {}
    )}`;
  })}</section> <div class="grid gap-4 svelte-m2ig6y">${each(buckets.rect, (info) => {
    return `<article class="bg-slate-800 p-2 border rounded border-slate-700 flex flex-col box-border overflow-hidden">${validate_component(SelectAppContent, "SelectAppContent").$$render(
      $$result,
      {
        info,
        state: $remoteState.get(info.serial)?.state
...
//# sourceMappingURL=_page.svelte-c35084ba.js.map
